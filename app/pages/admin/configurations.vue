<template>
    <div class="admin-subpage">
        <header class="subpage-header">
            ...
            <h1>{{ $t('admin.configurationsManagement') }}</h1>
        ...
        <div class="page-content">
            ...
            <div v-else-if="configurations.length === 0" class="empty-state">
                <h2>{{ $t('admin.noConfigurations') }}</h2>
            </div>

            <div v-else class="items-list">
                <div v-for="config in configurations" :key="config.id" class="item-card" @click="editItem(config)">
                    <div class="config-info">
                        <span class="config-key">{{ config.key }}</span>
                        <span class="config-value">{{ config.value || $t('admin.emptyValue') }}</span>
                    </div>
        ...
        <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
            <div class="modal glass-card">
                <div class="modal-header">
                    <h2>{{ editingId ? $t('admin.editConfiguration') : $t('admin.newConfiguration') }}</h2>
                    <button class="close-btn" @click="closeModal">&times;</button>
                </div>
                <form @submit.prevent="saveItem" class="modal-form">
                    <div class="input-group">
                        <label for="key">{{ $t('admin.configKey') }} *</label>
                        <input id="key" v-model="form.key" type="text" class="input-field" required>
                    </div>
                    <div class="input-group">
                        <label for="value">{{ $t('admin.configValue') }}</label>
                        <textarea id="value" v-model="form.value" class="input-field" rows="3"></textarea>
                    </div>
        ...
        <div v-if="showDeleteConfirm" class="modal-overlay" @click.self="showDeleteConfirm = false">
            <div class="modal glass-card modal-sm">
                ...
                <p class="confirm-text">{{ $t('admin.deleteConfigConfirm') }} <strong>{{ form.key }}</strong>?</p>
...
async function loadItems() {
    loading.value = true
    try {
        const data = await $fetch<{ configurations: Configuration[] }>('/api/admin/configurations')
...
function editItem(config: Configuration) {
    editingId.value = config.id
    form.value = { key: config.key, value: config.value || '' }
    formError.value = ''
    showModal.value = true
}
...
async function saveItem() {
    if (!form.value.key) {
        formError.value = t('admin.keyRequired')
        return
    }
    ...
    try {
        const body = { key: form.value.key, value: form.value.value || null }

        if (editingId.value) {
            await $fetch(`/api/admin/configurations/${editingId.value}`, { method: 'PUT', body })
        } else {
            await $fetch('/api/admin/configurations', { method: 'POST', body })
        }
...
async function deleteItem() {
    ...
    deleteError.value = ''
    try {
        await $fetch(`/api/admin/configurations/${editingId.value}`, { method: 'DELETE' })
...
.config-info { display: flex; flex-direction: column; flex: 1; min-width: 0; }
.config-key { font-weight: 600; font-size: 0.95rem; font-family: monospace; }
.config-value { font-size: 0.8rem; color: var(--color-text-muted); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
