export default defineEventHandler(async (event) => {
    ...
    try {
        ...
        const [oldFuelLog] = await pool.query(
        ...
        if (type === 'fuel') {
            ...
            if (oldFuelLog) {
                ...
                await pool.query(`DELETE FROM maintenances_logs WHERE transaction_id = ?`, [transactionId])
                await pool.query(
            ...
            if (oldFuelLog && (oldFuelLog.vehicle_id !== vehicleId || oldDateStr !== newDateStr)) {
                const oldSubsequentRefuels = await pool.query(
                    `SELECT id, transaction_id, date, odometer, fuel_volume, is_full_tank, is_previous_missed
                     FROM fuels_logs WHERE vehicle_id = ? AND transaction_id != ? AND date > ?
                     ORDER BY date ASC`,
            ...
            await pool.query(`DELETE FROM fuels_logs WHERE transaction_id = ?`, [transactionId])
            ...
            const existingMaintLogs = await pool.query(
