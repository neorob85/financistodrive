...
export default defineEventHandler(async (event) => {
    ...
    try {
        await withConnection(async (conn) => {
            ...
            const [config] = await conn.query('SELECT id FROM configurations WHERE id = ?', [id])
            if (!config) {
                throw createError({ statusCode: 404, message: 'Configurazione non trovata' })
            }

            await conn.query('DELETE FROM configurations WHERE id = ?', [id])
        })
